...
- **v2.9 교수동 증축+보스턴 모드**: ① **hum 2층 분리** — hum(1층)=안전 로비(인카운터 없음: 연구실 E/O/X·행정 라운지 V[yuengling/barolo/nightshift]·humta/colleague), **hum2(2층)=줌 강의동**(`encFor`의 ENC_HUM·RECLV 모두 hum2로 이동, 계단 S(20,12) WALKP 왕복). 줌 강의실 3실 = `lectern/lectern2/lectern3` NPC 이벤트전, **순서 해금**: lecture1(quizstu, ATK+5) → lecture2(evalghost, DEF+5) → lecture3(recpile·meetghost, 최대 MP+20) — S 플래그 `lecture2`/`lecture3` 신설. ② 캠퍼스 **교수동 진입로**(gen_maps: y21 동측 연장 x20-33 + 문앞 vline x33). ③ **테마5**(t.B5/R5 사암 석조+청동 지붕): `MAP_THEME` hum/hum2=5 + `tileImg`의 campus (30-36,17-19) 렉트 특례(외관 차별화). ④ **보스턴 모드**(`bostonStart`, 타이틀 버튼) — Lv20 박사로 3장 직행: gainLevels 공식 그대로 시뮬, 스킬 `lv<=20` 12종, keyboard, ₩150,000, `boss1/boss2/card/ch2first/ch3first=1`, checkTitles+`S.title='phd'`, hsq(20,14) 시작. ⑤ **스킬 계열 진화(접힘)** — SKILLS에 `sup:'상위id'` 필드(끝에 추가 — verify [7] 포맷 무영향): gungri→susin, jihaeng→bakhak, haeche→hwalyeon, bakhak→hwalyeon, dongdo→beopgo. playerCommand 액션 메뉴가 `sup` 습득 시 하위를 접고 「모든 스킬 보기」 토글(`SHOW_ALL_SK` 전역, 세이브 불필요) 제공. gainLevels 계승 안내·스탯 화면 ⤷표시. ⑥ **2차 리워크(시뮬 4,000회 동반)** — sadan: 타격마다 적 약화 1종(debAtk/weak/debSpd/noGuard, 2틱) / gakju: mp3·반격 ×2.0+약점, **`P.gakjuCd` 연속시전 불가**(쿨다운 미도입 시 1:1 보스 무한 반격 락 99-100% — 시뮬로 확인된 구멍) / gyeongse: **`P.buffGse` 신설 축**(buffAtk와 합연산 +25%p, 4턴) — **전투 상태 리셋 4지점+roundEnd에 buffGse·gakjuCd 동기 필수**(D단계 패턴). ⑦ **필드보스 기믹 실구현** — bossgoose/stumor는 문서·도전 메뉴 경고문에만 기믹이 있고 enemyAct 분기가 없던 버그(F단계 커밋부터 누락): 구스=`fieldbossRetal`(피격 45% ×1.0, 단일기·반격에만 반응 — **사단지심 다단히트는 면제** = 안티구스 테크)+3턴 날개폭풍(×1.25+둔화)+30% 둔화, 종양=4턴 고정뎀38+과로·35% 떠넘기기(×0.8+과로, **tryCounter 미호출** = 반격 불가)·반피 격노(×1.15). battleLoop/bossLoop의 playerAct 직후 `P.hp<=0` defeat 체크 추가(반격 사망 처리).
- **v2.10 VSC+정착 패치**: ① **VSC 방문연구동**(longwood 남서 rows13-15 cols2-7, 문 4,15 + 진입로 vline x4) — vsc1(로비, 인카운터 없음: 진탕집 NPC `jintang`=openShopWith[jin/gamjatang], NEBS 흔적 플레이버 `nebs`), vsc2(ENC_VSC2 1/16 Lv30+: pdyear1·grantghost), vsc3(ENC_VSC3 1/14 Lv33+: pdyear6·pighost, 상자 ₩30,000). 계단 S 패턴은 union과 동일(WALKP). 신규 적 4종 enemyAct 기믹: pdyear1=MP깎기, grantghost=고정뎀20+과로, pdyear6=debAtk·강타, pighost=침묵+MP누수. **시뮬(3,000회)**: 2층 Lv30 100% / 3층 맥북 Lv33 단일 ~100%·페어 73%·3팩 63%(도주 권장) ② **아이템 2종**: jin(₩9,000 HP45/MP25)·gamjatang(₩32,000 **`hpfull` 신설 — HP만 완전회복**, 전투/필드 사용처 모두 분기 + 상목 탈취 면제 음식) ③ **낚시 경제**: 골드 정산 삭제(무한 골드 차단), `S.fishMile` 마일스톤(3/6/9/12종 → SPD+2/LUK+3/HP+30/ATK+5) ④ **패배 거점 송환**: defeat()가 장별 거점으로(안암 연구실/dorm 3,2/yenching 3,2/hum 3,2 — vsc·intl 포함 맵 리스트 분기) ⑤ 보스턴 착륙 hsq(16,21) T정거장 앞(bostonStart 포함) ⑥ **전투 액션 UI**: 버튼=짧은 이름(한자 제거), 상세=`#actInfo` 반투명 박스(openMenu `items[].info`+`menuInfos` 전역, setFocus서 갱신, pointerenter 포커스) ⑦ ENC_INF labstu 32%로 상향. ⑧ **사단지심 최소타 바닥**(v2.10.1) — 각 타격 `Math.max(r.d, floor(atkNow*0.55*0.5))`: 다단히트가 flat def에 4번 깎여 저레벨서 평타보다 약하던 문제(五經골렘 def12 Lv6 사단 4뎀<평타11) 해결. def가 타격 절반 이상 먹을 때만(=저-atk 초반) 발동 → 후반 타격은 이미 def 넘어 무영향(구스 앵커 60→72% 소폭). dmgRange에 `minHit` 4번째 인자 추가, info()의 sadan 분기서 표시 동기.
...
| 3장 보스 구스 | **v2.9 기믹 실구현 후 재측정**: 무보급 Lv30 33% / Lv32 97% (반격·둔화 — 테크: 사단지심 위주 시 Lv30 72% — v2.10.1 최소타 바닥 반영) |
| 3장 스트레스 종양 | **v2.9 기믹 실구현 후 재측정**: 무보급 Lv30 35% / Lv32 82% (과로 압박 — 테크: 술이부작 위빙 시 Lv30 74%) |
