...
put(c,38,28,'H')   # [DEBUG] 디버그 돌탑 — 만지면 모든 스탯 +1000 (반복 가능)
...
put(c,33,19,'D')
# [v2.9] 교수동 진입로 — y21 동측 연장 + 문 앞 세로 (기존엔 도로가 없어 잔디로 걸어야 했다)
hline(c,21,20,33,'P')
vline(c,33,20,21,'P')
...
# ---------- [v2.9] 인문대 교수동 1층 22x14 — 로비(안전지대): 인카운터 없음, 쉼터·편의장치 ----------
...
# 우측: 행정실·휴게 라운지 (벽 col14 rows1-5 + row6 cols14-20, 입구 (17,6))
for y in range(1,6): hm[y][14]='B'
for x in range(14,21): hm[6][x]='B'
hm[6][17]='F'                         # 라운지 입구
put(hm,16,1,'V'); put(hm,19,2,'C'); put(hm,16,4,'C')   # 자판기(편의장치)·행정 책상
# 복도 장식: 게시판(K) + 동쪽 끝 2층 계단
for x in (10,12): hm[1][x]='K'
put(hm,20,12,'S')                     # 2층 계단 → hum2

# ---------- [v2.9] 인문대 교수동 2층 22x14 — 줌 강의동 (ENC_HUM 인카운터, 권장 Lv33+) ----------
h2=grid(22,14,'F'); border(h2,'B')
# 줌 강의실 3개 (A/B/C) — 각 단상 NPC에 말 걸면 이벤트전·고유 보상
for y in range(1,6): h2[y][7]='B'; h2[y][14]='B'
for x in range(1,21): h2[6][x]='B'
h2[6][4]='F'; h2[6][10]='F'; h2[6][17]='F'   # 각 강의실 입구
for rx in (1,8,15):
    for y in (3,4):
        for x in (rx+1,rx+3,rx+5): h2[y][x]='C'   # 수강생 책상(꺼진 줌 화면들)
# 복도 게시판 + 1층 계단 (동쪽 끝, 1층과 같은 위치)
